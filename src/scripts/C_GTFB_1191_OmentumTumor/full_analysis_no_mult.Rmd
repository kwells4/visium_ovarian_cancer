---
title: "C_GTFB_1191_OmentumTumor SC Analysis"
...
# Get marker genes
marker_genes <- read.csv(file.path(
  save_dir,
  "files/DE/Spatial_markers_SCT_cluster.csv"))

interesting_genes <- marker_genes %>%
  dplyr::filter(cluster %in% c(4, 5) & p_val_adj < 0.05 &
                  avg_log2FC > 0.75)

plot_genes <- unique(interesting_genes$gene)

gene_template <- "src/scripts/gene_plot_template.Rmd"

# Plot using gene plotting rmd template
gene_chunks <- plot_genes %>%
  map(~knit_expand(file = gene_template, gene = .x, group = "4_5_markers",
                   sample = sample))
```
