---
title: "A_955_OvarianTumor SC Analysis"
...
Not surprisingly, the T cells seem to be dispersed throughout (full disclosure, most of my background is in immune cells so I am more likely to focus on the immune populations).
...
SpatialDimPlot(seurat_data,
               cells.highlight = CellsByIdentities(object = seurat_data,
                                                   idents = "TCell"),
               cols.highlight = c(colors_celltype["TCell"], "grey50"))
```
...
seurat_data[[celltype]][[1]] <- factor(seurat_data[[celltype]][[1]],
                                   levels = c("Monocyte", "MSC", "Fibroblast",
                                              "Epithelial", "TCell"))

marker_genes_rna <- read.csv(file.path(save_dir,
                                    "files", "DE",
                                    paste0("Spatial_markers_",
                                    celltype, ".csv")),
                             row.names = 1)
...
rna_heatmap <- plot_heatmap(seurat_data, top10_rna$gene, celltype,
                            colors = colors_celltype)[[4]]

rna_heatmap_ave <- plot_heatmap(seurat_data, top10_rna$gene, celltype,
...
# Make a meta_df
sample_info <- FetchData(seurat_data, vars = c(celltype,
                                               clusters))
...
names(coloring) <- c(clusters, celltype)
...
meta_ave <- table(paste0(sample_info$SCT_celltype,
                         sample_info[[clusters]]))

meta_ave <- data.frame(meta_ave)
meta_ave$SCT_celltype <- sub("[0-9]$", "", meta_ave$Var1)
meta_ave[[clusters]] <- sub(".*(?=.$)", "", meta_ave$Var1, perl = TRUE)
meta_ave$Var1 <- NULL
meta_ave$Freq <- NULL
...
celltypes <- unique(seurat_data[[celltype]][[1]])
...
### Cell type {.tabset}
```{r "celltype gost"}
if(celltype == "SCT_celltype"){
  type <- "celltype"
} else if(celltype == "Spatial_celltype") {
  type <- "celltype_spatial"
}
# Path to output
