...
gost_output <- run_gost(seurat_de = marker_genes,
                        sources = c("GO:BP", "KEGG", "REAC", "TF", "HPA"))


results <- gost_output$gost_output$result
...
# Save gost output
save_gost(gost_output,
          save_dir_plots = file.path(out_dir_go, "celltype_spatial"),
          save_dir_text = file.path(out_dir_text, "celltype_spatial"))



de_to_pathview(seurat_de = marker_genes,
               path_id_list = path_id_list,
               out_dir = file.path(out_dir, "celltype_spatial"),
               seurat_assay = DE_assay,
               test_idents = celltype2,
               gen_id = gen_id)
