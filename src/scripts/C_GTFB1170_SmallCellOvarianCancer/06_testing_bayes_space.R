...
seurat_sce <- qTune(seurat_sce, qs=seq(2, 15), platform="Visium", d=npcs)
qPlot(seurat_sce)

cluster <- 11

set.seed(149)
seurat_sce <- spatialCluster(seurat_sce, q=cluster, platform="Visium", d=npcs,
                           init.method="mclust", model="t", gamma=2,
                           nrep=10000, burn.in=100,
                           save.chain=TRUE)

palette <- grDevices::colorRampPalette(RColorBrewer::brewer.pal(9, 
                           "Set1"))(cluster)

clusterPlot(seurat_sce, palette = palette)


seurat_enhanced <- spatialEnhance(seurat_sce, q=cluster, platform="Visium", d=npcs,
                                    model="t", gamma=2,
                                    jitter_prior=0.3, jitter_scale=3.5,
                                    nrep=1000, burn.in=100,
                                    save.chain=TRUE)

clusterPlot(seurat_enhanced, palette = palette)
...
# T cells - Seems a bit better in spatial
...
# Epithelial cells - no expression
